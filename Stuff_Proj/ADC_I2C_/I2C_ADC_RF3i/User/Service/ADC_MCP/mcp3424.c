...
void MCP3424_Init(I2C_HandleTypeDef* pI2C, UART_HandleTypeDef* pUART){
	...
	if(HAL_I2C_IsDeviceReady(pI2CHandle, (MCP3424add<<1), 1, 10) == HAL_OK){
		HAL_UART_Transmit(pUARTHandle, "MCP3424 Ready\r\n", 15, 10);
	}
	/////Write the configuration register to set the resolution
	HAL_I2C_Mem_Read(pI2CHandle, (MCP3424add<<1), configSet, 1, ADCdatarecv, 5, 10);

	//HAL_I2C_Master_Transmit(&hi2c1,(MCP3424add<<1),configSet,1,10);
	/////Read the data back
	HAL_I2C_Master_Receive(pI2CHandle, (MCP3424add<<1), ADCdatarecv, 5, 10);

	switch(Resolution){
	case 12:
		ADCval = 0;
		ADCval = (ADCdatarecv[1]<<0)|(ADCdatarecv[0]<<8);
	...
	case 16:
		...
		break;
	case 18:
		ADCval=(ADCdatarecv[2]<<0)|(ADCdatarecv[1]<<8)|(ADCdatarecv[0]<<16);
